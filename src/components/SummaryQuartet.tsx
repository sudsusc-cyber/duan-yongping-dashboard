...
export default function SummaryQuartet({ latest, delta, fetchedAt, portfolio }: Props) {
  ...
  return (
    <section className="max-w-[1280px] mx-auto px-4 sm:px-8 mb-6 sm:mb-8">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {/* ── AUM ─────────────────────────────────── */}
        <BanXin as="article" className="quartet-card">
          ...
          <div className="big-num quartet-bignum tnum mt-3" style={{ color: "var(--ink-1)" }}>
            ${(total / 1e9).toFixed(2)}
          ...
          <div className="mt-1 h-mono tnum" style={{ fontSize: 11, color: "var(--ink-3)" }}>
            13F · {reportTag} · {latest.totalPositions} pos
          </div>
          <div className="mt-4">
        ...
        <BanXin as="article" className="quartet-card">
          ...
          {portfolio?.dPct != null ? (
            <>
              <div
                className="big-num quartet-bignum tnum mt-3"
                style={{
              ...
              <div className="mt-1 h-mono tnum" style={{ fontSize: 11, color: "var(--ink-3)" }}>
                US 13F · {portfolio.basis} pos · ex-opt
              </div>
              <div className="mt-4">
              ...
              >
                <span>
                  {portfolio.coverage >= 0.9999
                    ? "full coverage"
                    : `cov ${(portfolio.coverage * 100).toFixed(0)}%`}
                </span>
                <span>
                  {fetchedAt
            ...
            <>
              <div className="big-num quartet-bignum tnum mt-3" style={{ color: "var(--ink-3)" }}>
                —
              </div>
              <div className="mt-1 h-mono tnum" style={{ fontSize: 11, color: "var(--ink-3)" }}>
                {fetchedAt
        ...
        <BanXin as="article" className="quartet-card">
          ...
          <div className="big-num quartet-datenum tnum mt-3" style={{ color: "var(--ink-1)" }}>
            {latest.filingDate}
          </div>
          <div className="mt-1 h-mono tnum" style={{ fontSize: 11, color: "var(--ink-3)" }}>
